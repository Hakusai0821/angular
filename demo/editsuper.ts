...
import { EventResultT, HttpProxyService, IEntity, LoaddingService, WorkTask,UrlParamService, WorkTaskDataSvc } from 'oc-restful';
import { ConfirmationService } from 'primeng/api';
import { ActivatedRoute } from "@angular/router";
import { FormArray, FormControl, FormGroup } from '@angular/forms';
import { concatMap, filter, never, Subject, Subscription, takeUntil, tap } from 'rxjs';
...
export abstract class EditSuper<T extends FormGroup, E extends IEntity>  {
  ...
  setWorktask(): any {
    setTimeout(() => {
      sessionStorage.setItem('worktaskdatas', JSON.stringify(this.worktaskdataSvc.worktaskdatas))
    }, 100);
    return this.worktaskdataSvc.getWorktask(this.router.url)
  }
  ...
  constructor(
    ...
    this.router_subscription = this.router.events.pipe(
      ...
      if (this.getWorktask() != null && this.time != '') {
        this.setWorktask().backupsdata[this.time] = this.options.getRawValue();
        sessionStorage.setItem('worktaskdatas', JSON.stringify(this.worktaskdataSvc.worktaskdatas))
      }
    })
  }
  ...
  backupsEntity() {
    if (this.getWorktask().type == 'edit' && this.getWorktask().backupsdata[this.time] != undefined) {
      setTimeout(() => {
  ...
  createEntity() {
    ...
    this.httpProxyService.RxCreateJwtJsonPost<EventResultT<E>>(this.getApiPath() + '/create', null).subscribe({
      next: (res) => {
        // 填充資料
        if (res.success)
  ...
  loadEntity() {
    ...
    this.httpProxyService.RxCreateJwtJsonGet<EventResultT<E>>(this.getApiPath() + '/' + this.pk).subscribe({
      next: (res: any) => {
        if (res.success) {
          // 分頁標籤
          if (this.getWorktask().type == '' && this.router.url.indexOf(this.pk + "/") == -1) {
            this.setWorktask().type = 'edit';
            if (this.titlelevel2key && !this.titlelevel3key) {
              this.setWorktask().id = res.data[this.titlekey]?.[this.titlelevel2key];
            } else if (this.titlelevel2key && this.titlelevel3key) {
              this.setWorktask().id = res.data[this.titlekey]?.[this.titlelevel2key]?.[this.titlelevel3key];
            }else{
  ...
  // 新增按鈕
  toNew() {
    this.router.navigate(["../null"], { queryParams: { amb: "a", time: new Date().getTime() }, relativeTo: this.activatedRoute });
  }
  ...
  doEdit() {
    this.initFormGroup();
    this.router.navigate(['.'], { relativeTo: this.activatedRoute, queryParams: { "amb": "m", time: new Date().getTime() } });
  }
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        if (this.amb == 'a') {
          ...
          this.initFormGroup();
          this.router.navigate(['.'], { relativeTo: this.activatedRoute, queryParams: { "amb": "b", time: new Date().getTime() } });
  ...
  ontabselect(item: any) {
    if (item == undefined || this.router.url.split('?')[0] == item.url.split('?')[0])
      return;
  ...
  formGroupRequired(FG: any = this.options) {
    ...
    if (!FG.valid) {
      for (let key in FG.controls) {
        ...
        if (formcontrol instanceof FormControl && formcontrol.errors?.["required"]) {
          console.error(key + " is FormControl")
          this.showError("請輸入" + this.translate.instant(`${className}.${key}`) + "\n");
          ...
          formcontrol.controls.forEach((elm: any, index) => {
            for (let item in elm['controls']) {
              if (elm.get(item).errors?.['required']) {
                ...
                let reqClassName = this.translate.instant(`${className}.${key}`);
                let req2lineName = this.translate.instant(`${className}.${item}`);
                this.showError(`${reqClassName}列表中第${index + 1}筆資料，${req2lineName}欄位必填\n`);
              }
            }
          })
  ...
  deleteEntity() {
    this.loaddingService.startLoading();
    this.httpProxyService.RxCreateJwtDelete<EventResultT<Boolean>>(this.getApiPath(), this.pk).subscribe({
      next: (res) => {
        if (res.success) {
          this.clearQueryData();
          this.doExit();
  ...
  //清空query數據的
  clearQueryData() {
    let a = this.worktaskdataSvc.worktaskdatas.filter((s: any) => s.type == 'query' && this.router.url.split('?')[0].indexOf(s.pk) != -1);
    if (a.length > 0) {
  ...
  saveEntity(data: E) {
    ...
    this.loaddingService.startLoading();
    this.httpProxyService.RxCreateJwtJsonPost<EventResultT<Object>>(this.getApiPath() + '/validate', data).pipe(
      tap(x => {
        if (!x.success) {
      ...
      concatMap(x => x.success ? this.httpProxyService.RxCreateJwtJsonPost<EventResultT<E>>(this.getApiPath(), data) : never()),
      tap(x => {
        if (x.success) {
  ...
  //儲存完之後清空路由的參數or導向到剛剛儲存的頁面但是切換成唯讀模式
  save_after(d: E) {
    ...
    this.clearQueryData();
    this.router.navigate(['..', d.pk], { relativeTo: this.activatedRoute, queryParams: { "amb": "b", time: new Date().getTime() } });
  }
  //提示確認框
  showError(msg = "") {
  ...
  addcol(cols: any, columns: ColType[] = this.columns) {
    ...
    cols.forEach((elm: any) => {
      ...
      if ((elm.cellEditor == "number" || elm.cellEditor == "float") && elm.valueGetter == undefined) {
        ...
        elm.valueGetter = (event: any) => {
          ...
          return this.formatNumber(event.data.controls[elm.field].value + "", elm.cellEditor == "number" ? 0 : 2);
        }
      }
      columns.push(elm);
    })
  ...
  formatNumber(value: any, precision: number = 0) {
    ...
    if (replacevalue.length > 1 && (replacevalue.substr(0, 1) == "0" && replacevalue.substr(1, 1) != ".")) {
  ...
  setUrlParamData(){
    let nowQueryData = this.worktaskdataSvc.worktaskdatas.find((s: any) => s.type == 'query' && this.router.url.split('?')[0].indexOf(s.pk + '/') != -1);
    if(nowQueryData != null){
      let nowQueryTime: string = nowQueryData.time == null ? "":nowQueryData.time
      let nowIndex: number = nowQueryData.backupsdata[nowQueryTime].datasource.data.findIndex((e:any)=>e.pk == this.pk);
      let nextData: any = nowQueryData.backupsdata[nowQueryTime].datasource.data[nowIndex+1];
      let previousData:any = nowQueryData.backupsdata[nowQueryTime].datasource.data[nowIndex-1];
